...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "CineList — Entre na sua conta" },
      { name: "description", content: "Entre ou crie sua conta para gerenciar sua watchlist." },
      { property: "og:title", content: "CineList — Entre na sua conta" },
      { property: "og:description", content: "Entre ou crie sua conta para gerenciar sua watchlist." },
...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (session) navigate({ to: "/watchlist" });
    });
    ...
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div className="relative mx-auto grid min-h-screen max-w-6xl grid-cols-1 items-center gap-12 px-6 py-12 lg:grid-cols-2">
        <div className="hidden lg:block">
          <Link
            to="/"
            className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card/50 px-4 py-1.5 text-xs uppercase tracking-[0.2em] text-muted-foreground backdrop-blur transition-colors hover:text-foreground"
          >
            <ArrowLeft className="h-3 w-3" /> Voltar para populares
          </Link>
        ...
        <div className="w-full max-w-md justify-self-center lg:justify-self-end">
          <div className="rounded-2xl border border-border bg-card/80 p-8 shadow-[var(--shadow-card)] backdrop-blur-xl">
            <div className="mb-8 flex items-center justify-between">
              ...
              <Link
                to="/"
                className="text-xs font-medium text-muted-foreground underline-offset-4 hover:text-foreground hover:underline lg:hidden"
              >
                Voltar
              </Link>
            </div>
